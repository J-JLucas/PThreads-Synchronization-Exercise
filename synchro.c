/* Concurrency Exercise based on the "Dining Philosopher" problem
 * Solution by Josh Lucas
 *
 * resources represent "chopsticks"
 * tasks t_i represent "philosophers"
 * Each philosopher spends x millisec "eating", followed by y millisec "thinking",
 * before getting hungry again.
 *
 * Program Use: synchro inputFile monitorTime NITER
 * where inputFile contains resource information (see repo .dat files),
 * monitorTime is frequencey monitor thread pauses simulation to print states
 * and NITER is the number of iterations each tasks must complete
 */
...
int getResIndex(char *string) {
	...
	// returns index of target resource, -1 if not found
...
void loadResources(char *tokens[MAX_NTOKEN], int i) {
	...
	for(int j = 1; j < i-1; j+=2) {
		int check = getResIndex(tokens[j]);
		if(check == -1) {			// resource doesn't exist
			...
			resArray[check]->value += atoi(tokens[j+1]);
			printf("r%d updated: = [%s|%d], ", check, (resArray[check])->name, (resArray[check])->value);
		}
	}
	putchar('\n');
	return;
...
void simSafety() {
	// performs some safety checks to make sure simulation can run
	
	// check if resources were created
	if(resCount == 0) {
		fprintf(stderr, "no resources created, simulation aborted\n");
		freeTasks();
		exit(1);
	}
	
	// ensure resource type needed by each task exists
	// if not abort as task will never finish
	for(int i = 0; i < taskCount; i++) {
		for(int j = 0; j < taskArray[i]->rcount; j++) {
			// this also doubles as updating the index
			taskArray[i]->rArray[j]->index = getResIndex(taskArray[i]->rArray[j]->name);
			if (taskArray[i]->rArray[j]->index  == -1){
				fprintf(stderr, "task [%s] requires resource that doesn't exist [%s], aborting...\n", taskArray[i]->name, taskArray[i]->rArray[j]->name);
